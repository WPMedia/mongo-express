...
var router = function (config) {
  ...
  appRouter.get('/db/backup', mongoMiddleware, routes(config).backupDatabase);
  appRouter.get('/db/restore', mongoMiddleware, routes(config).restoreDatabase);
  appRouter.get('/db/create', mongoMiddleware, routes(config).createDatabase);
  appRouter.get('/db/clone', mongoMiddleware, routes(config).cloneDatabase);
  ...
  //async calls
  appRouter.get('/db/:database/async/:action', function(req, res){
    
    if (mongo.connections['yadongtest']!==undefined){
        req.db=mongo.connections['yadongtest'];
    } else {
        mongo.connections['yadongtest']=mongo.mainConn.db('yadongtest');
        req.db = mongo.connections['yadongtest'];
    }

    backup({
      // uri: "mongodb://admin:fM4HlmHoa6bI@192.168.99.100:27017/admin",
      uri: "mongodb://admin:adminadmin@192.168.99.100:27017/yadongtest",
      root:  './../../dump',
      // root: __dirname + '/../../',
      collections:['system.users','system.version','me'],
      tar: 'dump.tar', // save backup inot this tar file
      callback:function(){
        res.status(200).json({
          database:req.params.database,
          action:req.params.action,
          file:"20160101-129001-admin-catalogs.tar.gz",
          status:"OK"
        });
      }
    });
