...
var router = function (config) {
  ...
  appRouter.all('*', function (req, res, next) {
    ...
    res.locals.mongodumpDir   = config.options.mongodumpDir;
  ...
  appRouter.get('/db/backup', mongoMiddleware, routes(config).backup);
  appRouter.get('/db/restore', mongoMiddleware, routes(config).restore);
  appRouter.get('/db/create', mongoMiddleware, routes(config).createdb);
  appRouter.get('/db/clone', mongoMiddleware, routes(config).clonedb);
  ...
  //backup database async calls
  appRouter.post('/db/:database/async/backup',mongoMiddleware,routes(config).backupDatabase);
  //backup database async calls
  appRouter.post('/db/:database/async/restore',mongoMiddleware,routes(config).restoreDatabase);
  //clone database async calls
  appRouter.post('/db/:database/async/clone',mongoMiddleware,routes(config).cloneDatabase);
