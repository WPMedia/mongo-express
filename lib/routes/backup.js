...
function getBackupFileName(dbname,collection){
    var now=new Date();
    ...
    return now.getFullYear()+month+day+"-"+hours+minutes+seconds+"-"+dbname+"-"+collection;
}
...
var routes = function (config) {
    ...
    exp.backupDatabase=function(req,res){
        ...
        var requestedCollections = req.body.collections;
        var collection,file,files=[];

        function backupSingleFile(){
            if (requestedCollections.length){
                collection=requestedCollections.shift();//remove the first element of the array and return it
                file = getBackupFileName(req.params.database,collection)+".tar.gz";
                backup({
                    db:req.db,
                ...
                res.status(200).json({
                    ...
                    action:'backup',
                    filePath:resolve(__dirname + '/../../../'+(config.options.mongodumpDir||'mongodump')),
                    files:files,
                    status:"OK"
                });
            }
