...
function getBackupFileName(dbname,collections,requestedCollections){
    var now=new Date();
    ...
    var fn = now.getFullYear()+month+day+"-"+hours+minutes+seconds+"-"+dbname;
    var coll="";
    if  (requestedCollections.length===collections.length){
        coll="-all";
    } else {
        coll="-"+requestedCollections.join("&");
    }
    return fn+coll;
...
var routes = function (config) {
    ...
    exp.backupDatabase=function(req,res){
        ...
        var file = getBackupFileName(req.db.databaseName,req.collections[req.db.databaseName],requestedCollections)+'.tar.gz'
        backup({
            db:req.db,
            // root:  './../../dump',
