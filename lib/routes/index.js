...
var backupRoute      = require('./backup');
var restoreRoute      = require('./restore');
var cloneRoute      = require('./clone');
...
var routes = function (config) {
  ...
  exp.backup            = databaseRoute(config).backup;//backup page
  exp.restore           = databaseRoute(config).restore;
  exp.createdb          = databaseRoute(config).createdb;
  exp.clonedb           = databaseRoute(config).clonedb;
  ...
  exp.backupDatabase    = backupRoute(config).backupDatabase;
  exp.restoreDatabase    = restoreRoute(config).restoreDatabase;
  exp.cloneDatabase     = cloneRoute(config).cloneDatabase;
